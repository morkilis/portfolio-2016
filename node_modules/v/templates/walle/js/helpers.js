...
;(function() {
    ...
    Handlebars.registerHelper('timelist', function(context, options){
        return '<ul class="time-list">' + context[this].map(function(mov) {
            return '<li class="schedule ' + (options === 'today' ? isTimeValid(mov.time) : 'valid') + '">' + mov.time + '</li>'
        }).join('\n') + '</ul>'
    })
    // TODO 这样做就失去了模板的意义
    Handlebars.registerHelper('menu', function(context, options) {
            ...
            return '\
                <nav>\
                    <div id="today" ' + (cur === 'today' ? 'class="active"' : '') + '>' + dateObj.today + '</div><div id="tomorrow" ' + (cur === 'tomorrow' ? 'class="active"' : '') + '>' + dateObj.tomorrow + '</div><div id="afterTomorrow" ' + (cur === 'afterTomorrow' ? 'class="active"' : '') + '>' + dateObj.afterTomorrow +'</div>\
                </nav>\
            '
    })
